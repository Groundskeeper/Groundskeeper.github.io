///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//
//	Anything labeled "NOTE:EXPERIMENTAL" is just what it says . . . EXPERIMENTAL!
//
//					|--TODO--|
//			GAMEPLAY:
//	* Fill out the level!
...
//
//			TECHNICAL:
//	* Replace calls for the timeslice or "backgroundsong" current time with gameEngine.GetCurrentTime()
//	* Make sure the landscape messages word wrap
//	* Implement a try-catch chain so we can automatically completely abort if we hit an unexpected error
//
//			OPTIMIZATION:
//
//			TOUCHSCREEN:
...
//	* The music doesn't pause when going into landscape mode while in MENU gamestate
//
//			TECHNICAL:
//	* FireFox Mobile
...
var gsMenu = {
	...
	Exit : function(){
		...
		// worldObjects array
		console.log("Releasing references to the items in the worldObjects array");
		for (var i = 0 ; i < this.worldObjects.length ; i++){
		...
		console.log("Emptying out the worldObjects array");
	...
	Update : function(){
		if (gameEngine.soundArray["backgroundsong"].sound.readyState != 4){
			console.log(this.timestamp + " - 'backgroundsong' readyState is " + gameEngine.soundArray["backgroundsong"].sound.readyState);
			return;
		}
		else if ((gameEngine.GetCurrentTime() <= 0 || gameEngine.soundArray["backgroundsong"].sound.ended) && !this.isStarting){
		...
		for (var i = 0; i < this.worldObjects.length; i++) {
			if (this.worldObjects[i] != null && this.worldObjects[i].active){
				try{
					...
					console.log(this.worldObjects[i].name + " - " + ex.message);
				}
			}
		}
	},
	...
	ChangeMenuScreen : function(menuScreenTitle){
		...
		console.log("Could not find menuScreen with title " + menuScreenTitle);
	},
...
var gsGame = {
	...
	Initialize: function(){
		console.log("Initializing GAME gamestate");
		...
		/*
		// Read in our XML files
		var credits = gameEngine.ReadJSONFile("Credits.json");

		// Read in our level file
		var jsonData = gameEngine.ReadJSONFile("./Levels/Level" + this.curLevel + ".json");

		// Get the filenames of our sounds
		this.allEvents = jsonData.events;
		...
		*/
		...
		console.log("Initialized GAME gamestate");
	},
	Exit : function(){
		console.log("===Unloading GAME gamestate===");
		...
		console.log("Releasing references to the enemyBomberObj");
		this.enemyBomberObj = null;

		// . . . and the enemyGorillaObj
		console.log("Releasing references to the enemyGorillaObj");
		this.enemyGorillaObj = null;

		// . . . and the enemyVehicleYellowObj
		console.log("Releasing references to the enemyVehicleYellowObj");
		this.enemyVehicleYellowObj = null;
		
		// . . . and the enemyVehiclePinkObj
		console.log("Releasing references to the enemyVehiclePinkObj");
		this.enemyVehiclePinkObj = null;
		
		// . . . and the enemyVehicleBlackObj
		console.log("Releasing references to the enemyVehicleBlackObj");
		this.enemyVehicleBlackObj = null;
		
		// . . . and the enemyVehicleBlueObj
		console.log("Releasing references to the enemyVehicleBlueObj");
		this.enemyVehicleBlueObj = null;

		// allEvents array
		console.log("Releasing references to the items in the allEvents array");
		for (var i = 0 ; i < this.allEvents.length ; i++){
			this.allEvents[i] = null;
		}
		console.log("Emptying out the allEvents array");
		...
		// screenOverlay
		console.log("Releasing screenOverlay reference");
		...
		console.log("===Unloaded GAME gamestate===");
	},
	GetInput : function(){
		...
		if (this.playerObj.curState != "dying" && 
			...
			if (gameEngine.isKeyPressed(37) || gameEngine.isJoystickMoved("shootingDiv", "left") ||
				...
				// Set the idleTimer
				this.playerObj.idleTimer = gameEngine.GetCurrentTime();
				...
				// DOWN and RIGHT
					...
					console.log(this.timestamp + " - " + "Could not determine the player state when attempting to shoot.");
				}
			}
		}
	...
	Update : function(){
		...
		if (!this.gamePaused && !this.gameOver){
			...
			for (var i = 0 ; i < this.allEvents.length ; i++){
				...
				var curTime = gameEngine.GetCurrentTime();
						...
						switch(eventAction){
							case "change_variable":
								...
								if (typeof(this[variable]) == "undefined"){
									console.log(this.timestamp + " - " + "Could not find a variable named " + variable + " to change.");
									break;
								}
							...
							case "spawn_item":
								...
								if (typeof(item) != "undefined" && item != null){
									...
									console.log(this.timestamp + " - " + "Could not spawn_item of type " + type + " at " + eventTimestamp);
								}
								break;
							case "move_actor":
								...
								if (actor != null){
									...
									console.log(this.timestamp + " - " + "Could not find an actor by the name of " + actorValue + " to move.");
								}
								break;
							case "speechortext":
								if (curTime >= (eventTimestamp + eventDuration)){
							...
							case "change_state":
								...
								if (!foundActor){
									...
									console.log(this.timestamp + " - " + "Could not find an object with the name " + actorName + " that was active to change their state to " + newState);
								}
								break;
							case "clear_enemies":
								...
								if (shortenList){
									for (var curActor = 0; curActor < this.worldObjects.length; curActor++) {
										if (this.worldObjects[curActor] == null){
											console.log("Shortening the worldObjects list. BEFORE: " + this.worldObjects.length);
											this.worldObjects.splice(curActor,1);
											console.log("Shortening the worldObjects list. AFTER: " + this.worldObjects.length);
											// Start over!
											curActor = -1;
										}
									}
			...
			for (var i = 0; i < this.worldObjects.length; i++) {
				if (this.worldObjects[i] != null && this.worldObjects[i].active){
					try{
						...
						console.log(this.timestamp + " - " + this.worldObjects[i].name + " - " + ex.message);
					}
				}
			}
			...
			if (!this.isCutscene && 
				(gameEngine.GetCurrentTime() - this.playerObj.idleTimer > this.playerMaxIdleTime) && 
	...
	Render : function(){
		...
		if (this.gameOver){
			...
			for (var c = 0 ; c < this.credits.length ; c++){
				if (this.credits[c].status == "new"){
					...
					this.credits[c].startTime = gameEngine.GetCurrentTime();
				}
				...
				if (this.credits[c].status == "inprogress"){
					...
					if (this.credits[c].duration > 0 && gameEngine.GetCurrentTime() > (this.credits[c].startTime + this.credits[c].duration) ){
		...
		if (gameEngine.debugMode){
			...
			ctx.fillText(gameEngine.GetCurrentTime(), 100, 20);
		}
	},
	...
	TriggerEvent : function(action, duration, otherData){
		var totalString = "" +
			"<event>" +
			"<timestamp>" + (gameEngine.GetCurrentTime()) + "</timestamp>" +			
...
var gameEngine = {
	...
	// Just like the variable name says
	hasUserTouchedDocumentYet : false,

	// Our canvas object
	canvas : null,

	// The view frustum to determine the bounds of what we consider eligible for rendering
	viewFrustum :  { minX: 0, minY : 0, maxX: 600, maxY : 400},
	...
	Initialize : function () {
		...
		this.viewFrustum.minX = 0;
		this.viewFrustum.minY = 0;
		this.viewFrustum.maxX = this.canvas.width;
		this.viewFrustum.maxY = this.canvas.height;
	...
	isJoystickMoved : function(elem, direction){
		...
		switch (direction.toLowerCase()){
			...
			case "right":
				...
				break;
			default:
				console.log("Could not determine the direction [" + direction + "] in the element [" + elem + "].");
				return false;
				break;
		}
	},
	...
	loadItemsJSON : function(items){
		...
		for (var i = 0 ; i < items.length ; i++){
			...
			try{
				...
				if (filename && !this.ResourceExists(filename)){
					throw {type: "Missing Resource", message: "Item file '" + filename + "' could not be found."};
				}
				else{
					// Grab the data from the file
					var curJSONobj = gameEngine.ReadJSONFile(filename);

					// Make sure the spritesheet exists
					if (!this.ResourceExists(curJSONobj.spritesheet)){
						throw {type: "Missing Resource", message: "Spritesheet '" + curJSONobj.spritesheet + "' for " + curJSONobj.name + " not found."};
					}

					// Do this operation "numcopy" times
					for (var j = 0 ; j < items[i].numcopy ; j++){

						// Create a new temp worldItem
						var temp = new worldItem();

						// Iterate through all the properties of this item
						for (var k = 0 ; k < Object.keys(curJSONobj).length ; k++){
							var curKey = Object.keys(curJSONobj)[k];

							// Don't do anything with the spritesheet property
							if (curKey == "spritesheet")
								continue;

							// Set the property for this worldItem
							temp[curKey] = curJSONobj[curKey];
						}
						
						// If it's "ai" or "collided" logic then it's an array of strings that we'll want to concatenate into one big string and set THAT as the property for this item
						if (temp.hasOwnProperty("ai")){
							var concatString = "";

							for (var k = 0 ; k < temp.ai.length ; k++){
								concatString += temp.ai[k];
							}

							temp.ai = concatString;
						}
						if (temp.hasOwnProperty("collided")){
							var concatString = "";

							for (var k = 0 ; k < temp.collided.length ; k++){
								concatString += temp.collided[k];
							}

							temp.collided = concatString;
						}
						// Concatenate the execute on finish logic for the frames
						for (var s = 0 ; s < temp.states.length ; s++){
							for (var f = 0 ; f < temp.states[s].frames.length ; f++){
								if (fortemp.states[s].frames[f].hasOwnProperty("executeonfinish")){
									var concatString = "";

									for (var e = 0 ; e < temp.states[s].frames[f].executeonfinish.length ; e++){
									}

									temp.executeonfinish = concatString;
								}
							}
						}

						// If this item doesn't have a "lasttimebulletfired" property then it doesn't need to Shoot();
						if(!temp.hasOwnProperty("lasttimebulletfired"))
							delete temp["Shoot"];

						// If it's not one of the missile launching enemies then get rid of the LaunchMissiles function as well
						if (temp.name != "enemyBomber" && temp.name != "enemyVehicleYellow")
							delete temp["LaunchMissiles"];

						// NOW set up the image property for this worldItem
						temp.image = this.AddImageToImageArray(curJSONobj.spritesheet);

						worldObjects.push(temp);
					}
				}
			}
			catch(ex){
		...
		// Our  array of worldItems
		return worldObjects;
	},
	// NOTE:EXPERIMENTAL
	loadSoundsJSON : function(sounds){
		for (var i = 0 ; i < sounds.length ; i++){
			var keyname = sounds[i].keyname;
			var filename = sounds[i].filename;
			var type = sounds[i].type;

			this.soundArray[keyname] = new sound(keyname, filename, type);
		}
	...
	// NOTE:EXPERIMENTAL
	loadBackgroundTilesJSON : function(tiles){
		var backgroundTiles = [];

		// Run through all the tiles that this level file is supposed to have . . .
		for (var i = 0 ; i < tiles.length ; i++){
			// The filename of this backgroundTile (e.g., "..//Images/Background_CitySkyline.png", etc.)
			var filename = tiles[i].filename;

			// Is this particular tile a startingBackground?
			if (tiles[i].isStartingBackground){
				// The Z-Plane of this backgroundTile
				var zplane = tiles[i].zplane;

				for (var j = 0 ; j < 2 ; j++){
					backgroundTiles.push(new backgroundTile(filename, parseInt(j * gameEngine.canvas.width), 0, parseInt(zplane)));
				}
			}
		}
...
function worldItem(){
	...
	this.update = function(){
		...
		if ((gameEngine.GetCurrentTime() - this.frameTimer) >= this.states[this.curState].frames[this.curFrame].duration){
			...
			this.frameTimer = gameEngine.GetCurrentTime();
	...
	this.render = function(){
		...
		if (this.animationWidth != 0 && 
			...
			this.x > gameEngine.viewFrustum.minX && 
			this.x < gameEngine.viewFrustum.maxX && 
			this.y > gameEngine.viewFrustum.minY && 
			this.y < gameEngine.viewFrustum.maxY)
		{
	...
	this.changeState = function(nameOfState){
		...
		if (typeof(this.states[nameOfState]) == 'undefined'){
			console.log(this.timestamp + " - No state by the name of " + nameOfState + " for item with the name of " + this.name);
			return;
		}
		...
		if (this.active && (this.curState.indexOf("leaving") != -1)){
			...
			if (nameOfState != "dying"){
				...
				console.log(gsGame.timestamp + " - " + this.name + " cannot change the state to '" + nameOfState + "' while currently '" + this.curState + "'!");
				return;
			}
		}
	...
	this.Shoot = function(direction, originX, originY, speedX, speedY){
		...
		if (gameEngine.GetCurrentTime() >= nextAvailableFireTime){
			...
			if (typeof(ourBullet) != "undefined" && ourBullet != null){
				...
				this.lastTimeBulletFired = gameEngine.GetCurrentTime();
				...
				switch(direction){
					...
					case "shoot_downright":
						...
						break;
					default:
						console.log("Could not determine the direction of " + direction + " while attempting to Shoot() for " + this.name);
						break;
				}
				...
				console.log(gsGame.timestamp + " - Could not find an object with the name " + (	(this.name == "mouse" || this.name == "rabbit") ? "playerBullet" : this.name + "Bullet") + " to Shoot().");
			}
		}
	}
	this.LaunchMissiles = function(numToLaunch){
		for (var numMissiles = 0 ; numMissiles < numToLaunch ; numMissiles++){
			...
			if (missile == null || reticle == null){
				console.log(this.timestamp + " - " + "On iteration " + numMissiles + " missile found was " + missile + " and reticle was " + reticle);
				continue;
			}
	...
	this.unload = function(){
		...
		for (var s in this.states){
			...
			if (this.states[s].frames != null){
				...
				console.log("Releasing " + this.states[s].frames.length + " stateFrame references for '" + this.states[s].name + "' for " + this.name);
			...
			if (this.states[s].name == ""){
				console.log("THIS STATE NAME IS BLANK!\n" + s);
			}

			console.log("Releasing '" + this.states[s].name + "' state reference for " + this.name);
			this.states[s] = null;
		}
	}
}
...
function sound(filename, type){
	...
	this.play = function (resume) {
		...
		if (this.type == "music"){
			console.log("RESTARTING MUSIC and resetting worldobjects!!");
	...
	this.unload = function(){
		console.log("Deleting and removing reference for sound '" + this.sound.src + "'");
...
function menuScreen(title, options){
	...
	this.unload = function(){
		console.log("Releasing menuOptions for menuScreen with title of '" + this.title + "'");
		for(var i = 0 ; i < this.menuOptions.length ; i ++){
			this.menuOptions[i] = null;
		}

		console.log("Emptying out the menuOptions array for menuScreen with title of '" + this.title + "'");
